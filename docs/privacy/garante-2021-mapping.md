...
| | |
|---|---|
| Implementation | The banner renders both as native `<button>` elements built by the same function, with the same class (`k`) and the same attributes; `data-a`/`data-r` only wire the click handlers (`services/tracker/src/banner/template.ts`). Parity holds **by construction** whatever the operator customises: the theme has a single button colour pair (`colors.accent`/`colors.accentText`, no "secondary" style) and the compiled stylesheet has one rule for `.k` (`Consent\Application\BannerStylesheet`); custom CSS can reach the buttons only through the public name `.button`, which always means both, because the selector allowlist refuses everything that could single one out — attribute selectors, `:first-child`/`:last-child`/`:nth-*`/`:only-*`/`*-of-type`, `:not()`, `:has()`, `:is()`, `:where()`, `+`, `~`, ids, elements — and the property allowlist refuses `display`, `visibility`, `opacity`, `order`, `flex-direction`, positioning and transforms (`Consent\Application\CustomCssCompiler`, [../integration/consent-banner.md](../integration/consent-banner.md#custom-css)). Closing (`×`) and Escape are recorded as `dismiss` and are treated as a **rejection**, not as acceptance. Scrolling is not consent — there is no scroll listener in the consent path. There is no blocking overlay (a `center` backdrop is only painted, as a shadow, and never captures clicks). |
| Tests | tracker `test/banner.test.ts` → "renders Accept and Reject as native buttons with identical styling" (same tag, class, attributes, no inline style, only two children of the actions row); `test/consent.test.ts` → "close button and Escape count as dismiss and reject"; `Analytics\Tests\Unit\Consent\CustomCssCompilerTest` → hostile selectors and declarations, "nothing accepted can tell Accept from Reject"; `BannerStylesheetTest::testAcceptAndRejectShareOneRule`; e2e `consent-layout.spec.ts` → "custom CSS styles Accept and Reject identically" (every computed style equal) |

### B3. The choice must be as easy to withdraw as to give

| | |
|---|---|
| Implementation | `analytics.consent.open()` reopens the banner; `[data-analytics-consent]` and links to `#analytics-consent` do it declaratively; an optional floating button (`show_floating_reopen`) is shown after a choice. `analytics.consent.set('rejected')` withdraws directly, and `analytics.consent.forget()` additionally erases the stored data. The floating button's look is set per device class (`theme.reopen.desktop` / `theme.reopen.mobile`: text, icon, icon and text, or hidden); every visible variant is a native button with an `aria-label` carrying the reopen text, so the icon-only one is still named. Custom CSS cannot hide or move it (`display`, `visibility`, `opacity`, positioning, `margin` on `.reopen` are refused). Hiding it on a device class is an operator choice that the documentation ties to keeping a `data-analytics-consent` link on every page ([../integration/consent-banner.md](../integration/consent-banner.md#reopening)); the declarative links and `consent.open()` work regardless of the theme. |
| Tests | tracker `test/api.test.ts` → "[data-analytics-consent] and #analytics-consent links reopen preferences"; `test/banner.test.ts` → "opens via consent.open(), focuses the dialog and returns focus on close", → "shows a floating reopen button after a choice when configured", → "renders the reopen icon from the configured path, hidden from assistive technology"; `test/consent.test.ts` → "forget sends /forget, deletes ids and rejects"; `BannerStylesheetTest::testReopenVariantsPerDevice`; e2e `consent-layout.spec.ts` → "the reopen button shows an icon on phones and text on desktop" (named by its aria-label, and it reopens the banner) |
...
| | |
|---|---|
| Implementation | Texts are stored per locale; the banner picks `<html lang>`, falls back to the configured default locale and then to the first available one. It renders in an open shadow root with `role="dialog"`, `aria-modal="false"`, `aria-labelledby`/`aria-describedby`, a focusable dialog, focus return on close, `prefers-reduced-motion` and `forced-colors` support. The API refuses a theme whose text/background, button, link or reopen-button contrast is below 4.5:1 (`ContrastChecker`), and custom CSS whose text colours are translucent or fall below 4.5:1 against the colours they are drawn on. |
| Tests | tracker `test/banner.test.ts` → "exposes dialog semantics with labelled and described content", → "falls back to the first locale when the default one is missing", → "renders in an open shadow root prepended to body with constructable styles", → "renders the reopen icon from the configured path, hidden from assistive technology"; `Analytics\Tests\Functional\Consent\ConsentApiTest::testValidationAndContrast`, `ConsentThemeApiTest::testValidationNamesEveryField`, `CustomCssCompilerTest` (contrast cases); `tests/fixtures/banner-css/*.css` snapshots (`prefers-reduced-motion`, `forced-colors`) |
