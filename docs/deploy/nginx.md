...
`OPS_TOKEN` is generated by `secrets:generate`.

`POST /_ops/opcache-reset` is the only endpoint under it. It answers **only** when `OPS_TOKEN` is set
(otherwise `404`), the client address is loopback (otherwise `403 forbidden`) and the request carries
`Authorization: Bearer <OPS_TOKEN>` (otherwise `401`). It calls `opcache_reset()` and replies
`{"reset": true|false, "enabled": true|false}`. Use it after a release when the web server keeps
serving the previous release from OPcache and you cannot reload PHP-FPM:

```sh
curl -fsS -X POST -H "Authorization: Bearer $OPS_TOKEN" http://127.0.0.1/_ops/opcache-reset
```

With `$realpath_root` in the vhost (see above) this is rarely needed: each release has its own real
path, so its files are cached under new keys. Any other path under `/_ops/` returns `404`.
