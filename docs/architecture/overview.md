...
- **Automatic events** (outbound links, file downloads, form submits). The site setting `auto_events`
  and the `auto` block of `window.__an_cfg` exist, but the tracker never reads them.
- **`POST /_ops/opcache-reset`**. `OPS_TOKEN` is generated by `secrets:generate` and the example nginx
  vhosts reserve `/_ops/`, but no route handles it; such a request falls through to the SPA.
