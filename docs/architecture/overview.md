...
| `MAILER_DSN` | empty | optional mailer (Symfony Mailer DSN: `smtp://`, `smtps://`, `sendmail://`); enables password reset by email and email changes. See [../operations/mail.md](../operations/mail.md); check with `mail:test` |
| `MAIL_FROM` | `analytics@localhost` | sender address; must be one the SMTP account may send as |
| `MAIL_FROM_NAME` | `Analytics` | sender display name |
...
- **Email reports.** `symfony/mailer` is used for password resets and email-change confirmations
  only. Invitations are never mailed; they are copy-paste links.
